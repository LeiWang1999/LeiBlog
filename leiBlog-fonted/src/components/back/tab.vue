<template>
  <v-container grid-list-xs>
    <v-tabs show-arrows color="primary" height="40px">
      <v-tab v-for="(tab, i) in tabs" :key="tab.name" :to="tab.href" :name="i" @contextmenu="show">
        {{tab.name}}
        <v-btn
          fab
          icon
        ...
        >
          <v-icon size="14" :color="$route.path==tab.href?'primary':''">mdi-close</v-icon>
        </v-btn>
      </v-tab>
    </v-tabs>
    <v-divider></v-divider>
    <v-menu v-model="showMenu" :position-x="x" :position-y="y" absolute offset-y min-width="110">
      <v-list dense>
